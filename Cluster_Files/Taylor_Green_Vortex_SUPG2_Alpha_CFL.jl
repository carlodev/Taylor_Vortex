...
for i = 1:1:length(N_CFL)
  ...
  ρ∞ = 1 #GeneralizedAlpha parameter, ρ∞=1 no dissipation, ρ∞ = 0 max dissipation
  ...
  global  iteration, Ntimestep, CFL
  ...
  createpvd("TV_2d") do pvd
    for (xh_tn, tn) in sol_t
      ...
      global iteration
      iteration += 1
      println("it_num = $iteration of $Ntimestep CFL = $CFL\n")
      uh_tn = xh_tn[1]
      ph_tn = xh_tn[2]
      ωh_tn = ∇ × uh_tn
      ωn = ωh_tn ./ ω₀
  ...
  @save "TG_CFL_0p32_AlphaM_rho_1_order_1_mesh_64_CFL_$CFL.jld2" err
