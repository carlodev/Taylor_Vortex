module StabilizeComputationCart


using LinearAlgebra
export h_compute2D, stabilization_coefficients
...
function h_compute2D(model, velocity, t)
...
return h_element(t), u_mid(t)

end



function stabilization_coefficients(h_element,nu,u_mid;r=2)
n_ele = length(h_element)
tau_su = zeros(n_ele)
tau_ps = zeros(n_ele)
tau_bk = zeros(n_ele)
    for i = 1:1:n_ele
    ...
    return (tau_su(t), tau_ps(t), tau_bk(t))
end
