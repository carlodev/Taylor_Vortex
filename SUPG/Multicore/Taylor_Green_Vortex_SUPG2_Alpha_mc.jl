...
function run_TaylorGreen(parts)
  ...
  model_nodes = map_parts(Ω.models) do model
    DiscreteModel(Polytope{0}, model)
  end
  ...
  nls = NLSolver(ls, show_trace=true, method=:newton, linesearch=MoreThuente(), iterations=30, ftol=1e-8)
...
partition = (2, 2)
prun(run_TaylorGreen, mpi, partition)
#mpiexecjl --project=. -n 4 julia Taylor_Green_Vortex_SUPG2_Alpha_mc.jl